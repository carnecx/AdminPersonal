...
namespace AdminPersonal.Pages.Oferentes
{
    public class EditarModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            { ViewData["Error"] = "La identificaci�n es obligatoria."; return Page(); }

            if (string.IsNullOrWhiteSpace(Oferente.TipoIdentificacion))
            { ViewData["Error"] = "El tipo de identificaci�n es obligatorio."; return Page(); }

            if (string.IsNullOrWhiteSpace(Oferente.NombreCompleto))
            { ViewData["Error"] = "El nombre completo es obligatorio."; return Page(); }

            if (Correos.Count == 0)
            { ViewData["Error"] = "Debe ingresar al menos un correo electr�nico."; return Page(); }

            foreach (var correo in Correos)
                if (!correo.Contains("@") || !correo.Contains("."))
                { ViewData["Error"] = $"El correo '{correo}' no tiene formato v�lido."; return Page(); }

            if (Telefonos.Count == 0)
            { ViewData["Error"] = "Debe ingresar al menos un tel�fono."; return Page(); }

            if (await _oferenteService.IdentificacionExisteAsync(Oferente.Identificacion, Oferente.id_oferente))
            { ViewData["Error"] = "Ya existe otro oferente con esa identificaci�n."; return Page(); }
