...
namespace AdminPersonal.Repository
{
    public class BitacoraRepository
    {
        ...
        public async Task<int> ContarAsync(string? usuario, string? descripcion)
        {
            ...
            return await conexion.QueryFirstAsync<int>(sql, new { usuario, descripcion });
        }
        ...
        public async Task<IEnumerable<Bitacora>> ListarAsync(string? usuario, string? descripcion, string orden, int pagina, int tamanoPagina)
        {
            ...
            var sql = @"
                SELECT b.id_bitacora, b.fecha, b.id_usuario, u.nombre_usuario, b.descripcion
                FROM bitacora b
                INNER JOIN usuario u ON u.id_usuario = b.id_usuario
                WHERE (@usuario IS NULL OR @usuario = '' OR u.nombre_usuario LIKE CONCAT('%', @usuario, '%'))
                  AND (@descripcion IS NULL OR @descripcion = '' OR b.descripcion LIKE CONCAT('%', @descripcion, '%'))";
            ...
            return await conexion.QueryAsync<Bitacora>(sql, new { usuario, descripcion, tamanoPagina, offset });
        }
    }
}
